...
desc "Bundle all the cookbooks for chef-solo distribution by merging cookbooks & site-cookbooks"
task :bundle do
  temp_dir = File.join(Dir.tmpdir, "chef-upload-cookbooks")
  tarball_dir = File.join(TOPDIR, "pkgs")
  FileUtils.mkdir(temp_dir)
  FileUtils.mkdir_p(tarball_dir)
  cookbook_folders = [ 'cookbooks', 'site-cookbooks' ]
  cookbook_folders.each do |folder|
    file_path = File.join(TOPDIR, folder, '.')
    FileUtils.cp_r(file_path, File.join(temp_dir, '/cookbooks/')) if File.directory?(file_path)
  end
  system('tar', '-C', temp_dir, '--exclude', '.git*', '-czf', File.join(tarball_dir, "./cookbooks.tgz" ), ".")
  FileUtils.rm_rf temp_dir
end
